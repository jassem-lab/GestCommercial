...
<table class="table mb-0">
    ...
    <tbody>
        <?php
	$reqnom="select * from delta_banque_client where client=".$client;
...
$(".btnModif1").on("click", function() {
    ...
    if (confirm('Confirmez-vous la modification?')) {
        ...
        $.post("page_ajax/ajax_modifier_banqueclient.php", variable, function(data, status) {
            if (status == "success") {
                ...
                xmlhttp_listemps.open("POST", "page_json/json_listebanques_client.php", true);
...
$(".btnDelete").on("click", function() {
    ...
    if (confirm('Confirmez-vous la suppression?')) {
        var variable = "id=" + id;
        $.post("page_ajax/ajax_supprimer_banqueclient.php", variable, function(data, status) {
            if (status == "success") {
                if (window.XMLHttpRequest) {
                    xmlhttp_listemps = new XMLHttpRequest();
                } else {
