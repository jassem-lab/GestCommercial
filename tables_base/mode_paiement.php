...
<div class="col-xl-12">
    ...
    <form name="SubmitContact" class="row mb-3" method="post" action="" onSubmit="" style='margin-top : 50px ; '>
        <div class="col-xl-3">
            ...
            <select class="form-control select2" name="mode" id="mode">
                ...
                <?php
                      echo  $reqc="select * from delta_mode_paiement" ;
                        $queryc=mysql_query($reqc);
                        while($enregc=mysql_fetch_array($queryc)){
                        ?>
                <option value="<?php echo $enregc['id']; ?>" <?php if($mode==$enregc['id']) {?> selected <?php } ?>>
    ...
    <table class="table mb-0">
        ...
        <tbody>
            <?php 
            $reqFP ="select * from delta_mode_paiement where 1=1".$reqMode; 
